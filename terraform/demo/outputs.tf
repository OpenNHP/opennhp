...
output "demo_nhp_cert" {
  ...
  #
  # nonsensitive() is safe here because:
  # - The leaf cert is generated by Terraform and is not secret
  # - The CA cert is a PUBLIC certificate meant to be distributed to clients
  #   (only the CA private key is sensitive, not the cert itself)
  value = local.stealth_ca_enabled ? nonsensitive("${tls_locally_signed_cert.demo_nhp[0].cert_pem}${local.secrets["stealth_ca_cert"]}") : ""
}
