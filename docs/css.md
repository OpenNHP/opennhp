...
在多个框架并存或者抓取第三方编写 HTML 时，我们不愿看到：

- 按照我们的 CSS 编写的 HTML 结构应用了其他框架的样式；
- 抓取过来的 HTML 结构存在一些 class 相同的元素，意外地应用了 Amaze UI 的样式。

Amaze UI 是爱好和平的，我们在 class 上加了 `am` 这一命名空间，防御入侵，也不去侵入别人的结构。
