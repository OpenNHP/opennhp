...
class FirewallRulesTest(unittest.TestCase):
    def test_rules_and_input_validation(self):
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            log.write_text("")
            result = subprocess.run(["bash", str(script)], env=dict(env, FAIL_TEST="1", RMEM_MAX="212992"), capture_output=True)
            self.assertNotEqual(result.returncode, 0)
            self.assertNotIn("sysctl -w", log.read_text())
            self.assertNotIn("-restore --wait --noflush", log.read_text())
            for override in ({"NHP_TRUSTED_PEERS": ""}, {"NHP_TRUSTED_PEERS": "0.0.0.0/0"},
                             ...
                             {"NHP_KNOCK_GLOBAL_RATE_BURST": "20000"},
