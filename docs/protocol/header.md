...
| 16 | 8 | **Counter** | 64-bit big-endian nonce and transaction tracker. The counter occupies bytes 4–11 of the 12-byte GCM nonce; bytes 0–3 are zero-padded. See [`NonceBytes`](https://github.com/OpenNHP/opennhp/blob/main/nhp/core/scheme/curve/header.go#L77). Monotonically increments per encryption; receivers reject stale counters for that session. |
...
| 56 / 88 | 80 | **IBC Identity Ciphertext** | AEAD-encrypted IBC identity block: a fixed 64-byte plaintext slot (`MaximumIdentitySize`) — zero-padded when the identity is shorter, or entirely zero in PKI mode — followed by a 16-byte tag. |
