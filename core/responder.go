...
type PacketParserData struct {
	...
	decryptedMsgCh chan<- *PacketParserData //本次解密之后把明文消息发出去
	feedbackMsgCh  chan<- *PacketParserData //准备接收外部的消息
	Error          error
}
...
func (ppd *PacketParserData) validatePeer() (err error) {
	...
	log.Debug("tzy headType:%s,validatePeer pubkey: %s,EphermeralBytes:%s", HeaderTypeToString(ppd.HeaderType), base64.StdEncoding.EncodeToString(ppd.deviceEcdh.PublicKey()),
	...
	default:
		KeyByteSlice := key[:]
		log.Warning("tzy--ppd.Ciphers.GcmType:%d,&key:%s,NonceBytes:%s,StaticBytes:%s", ppd.Ciphers.GcmType, base64.StdEncoding.EncodeToString(KeyByteSlice), base64.StdEncoding.EncodeToString(ppd.header.NonceBytes()), base64.StdEncoding.EncodeToString(ppd.header.StaticBytes()))
