...
func (d *Device) createMsgAssemblerData(md *MsgData) (mad *MsgAssemblerData, err error) {
	if md.PrevParserData != nil {
		...
		log.Debug("tzy createMsgAssemblerData mad.RemotePubKey:" + base64.StdEncoding.EncodeToString(mad.RemotePubKey))
		// init packet buffer
...
func (mad *MsgAssemblerData) setPeerPublicKey(peerPk []byte) (err error) {
	...
	log.Debug("tzy 	HeaderType:%s encrypted pubkey: %v, output: %v", HeaderTypeToString(mad.HeaderType), base64.StdEncoding.EncodeToString(mad.deviceEcdh.PublicKey()), static)
