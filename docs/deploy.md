...
|-----------|-----------|-------------|
| `PrivateKeyBase64` | All | Private key (static, requires restart). Either a plain base64 key or a sealed blob (`v1$…`) produced by `<daemon> seal`; a sealed blob is decrypted at startup with a passphrase from `NHP_KEY_PASSPHRASE_FILE` (preferred) or `NHP_KEY_PASSPHRASE`. See the per-daemon `config.toml` comments. |
