...
type serverMetrics struct {
	...
	packetsDropped   *metrics.CounterVec // stage=too_short|blocked|precheck|rate_limited|per_ip_conn_limit|conn_limit|conn_duplicate|parse|validate|decrypt|queue_full
...
func newServerMetrics(s *UdpServer, startTime time.Time) *serverMetrics {
	...
	sm := &serverMetrics{
		...
		packetsDropped: reg.NewCounter("nhp_server_packets_dropped_total",
			"Inbound packets discarded before becoming a decrypted message, by stage "+
				"(too_short, blocked, precheck, rate_limited, per_ip_conn_limit, conn_limit, conn_duplicate, parse, validate, decrypt, queue_full).", "stage"),
	...
	for _, stage := range []string{
		"too_short", "blocked", "precheck", "rate_limited", "per_ip_conn_limit", "conn_limit", "conn_duplicate", // recvPacketRoutine, pre-decryption
...
// "precheck", "rate_limited", "per_ip_conn_limit", "conn_limit", "conn_duplicate") and the packetToMsgRoutine
