...
func (a *UdpDevice) connectionRoutine(conn *UdpConn) {
	...
	for {
		...
		case pkt, ok := <-conn.ConnData.RecvQueue:
			...
			log.Debug("接收数据时的 publicKey:%s", a.device.PublicKeyBase64())
...
func (a *UdpDevice) SendDHPRegister(doId string, policy common.DHPPolicy, dataKey string) {
	log.Debug("DHP数据封装器开始工作")
	serverPeer := a.GetServerPeer()
	wrappedKey := dataKey //加密
	// fmt.Println("serverPeerMap.size:" + )
	...
	if result {
		fmt.Println("文件加密及注册：成功")
	} else {
		fmt.Println("文件加密及注册：失败")
	}
}
...
func (a *UdpDevice) SendNHPDRG(server *core.UdpPeer, msg common.DRGMsg) bool {
	...
	result = func() bool {
		...
		log.Info("SendNHPDRG 返回结果：%v", string(dakMsgString))
		if dakMsg.ErrCode != 0 {
	...
	log.Info("SendNHPDRG 发送成功，最后返回结果：%v", result)
	return result
}
